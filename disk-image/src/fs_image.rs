...
fn main() {
    ...
    coolfs.create_file("/LOGS/BROWSER-ENGINE-HOST.TXT", BROWSER_ENGINE_HOST_LOG);
    ...
    let mut guidemo_payload_bytes: Option<Vec<u8>> = None;
    if let Some(guidemo_path) = guidemo_elf {
        ...
        guidemo_payload_bytes = Some(guidemo_bytes);
    }
    ...
    let guidemo_pkg_text = if let Some(guidemo_payload) = guidemo_payload_bytes.as_ref() {
        let mut payload_file = packages
            .create_file("guidemo.elf")
            .expect("failed to create guidemo.elf");
        payload_file.truncate().unwrap();
        payload_file
            .write_all(guidemo_payload)
            .expect("failed to write guidemo.elf");
        coolfs.create_file("/Packages/guidemo.elf", guidemo_payload);
        phase70_guidemo_package(guidemo_payload)
    } else {
        phase70_guidemo_package(&[])
    };
    ...
    guidemo_pkg
        .write_all(guidemo_pkg_text.as_bytes())
        .expect("failed to write guidemo.pkg");
    coolfs.create_file("/Packages/guidemo.pkg", guidemo_pkg_text.as_bytes());
...
fn phase70_guidemo_package(payload: &[u8]) -> String {
    if payload.is_empty() {
        return String::from("id=app.phase25.guidemo\nname=Packaged GUI Demo\ncommand=pkgdemo\nversion=1.0\nicon=P5\ncategory=Development\npermission=desktop\nexec=/bin/guidemo\naliases=package,demo,phase25\nassociations=P25\ndepends=\nmin_os_version=1\n");
    }
    format!(
        "id=app.phase25.guidemo\nname=Packaged GUI Demo\ncommand=pkgdemo\nversion=1.0\nicon=P5\ncategory=Development\npermission=desktop\nexec=/bin/pkgdemo\naliases=package,demo,phase25\nassociations=P25\ndepends=\nmin_os_version=1\npayload=/bin/pkgdemo|/Packages/guidemo.elf|{}|755\n",
        sha256_hex(payload)
    )
}
...
const BROWSER_ENGINE_HOST_LOG: &[u8] =
    b"coolOS browser engine host log\nphase=97\nstatus=not-started\nbackend=test\n";

const SDK_README: &[u8] = b"coolOS SDK\n\nABI version: 14\nUserspace apps are no_std Rust ELF64 binaries linked with userspace/libcool.\nUseful APIs: process::spawn_args, process::spawn_fds_args, thread::{spawn,spawn_tls,join,set_tls_base,tls_key_create,futex_wait,futex_wake,PThreadMutex,PThreadCondvar,PThreadOnce}, posix::{init_main_thread,pthread_create,pthread_join,pthread_exit,pthread_self,pthread_mutex_lock,pthread_cond_wait,pthread_once,pthread_key_create,pthread_setspecific,errno,set_errno,gettid,sched_yield,nanosleep}, dynlink::load, dynlink::load_with_deps, dynlink::Workspace for /lib ET_DYN objects, memory::{mmap,mmap_file,mprotect}, evented::poll, tty::{size,set_mode,enter_raw_mode}, fs::{stat,rename,chdir,getcwd,sync}, io::{open,open_flags,create,pipe}, gui::Window.\nShared-object experiments live under /lib; Phase 77 file-backs read-only /lib ET_DYN segments through mmap_file while Phase 76 dependency graphs still load /lib/libphase76main.so plus DT_NEEDED /lib/libphase76dep.so with RELA relocations, global symbol lookup, TLS records, W^X mprotect, and init-array execution.\nPOSIX file-runtime coverage includes /TMP, open/create/open_flags shims, fd read/write/close, read-only file-backed mmap, and /bin/mmapdemo for verification.\nPackage manifests live under /Packages, must be signed with pkg sign or pkg sign-as, and install transactionally into /APPS/<command>/APP.CFG with an OWNER.TXT trust record.\nUse payload=<target>|<source>|<sha256>|<mode> to copy real files, such as an ELF from /Packages into /bin, with installed payload hashes verified by pkg verify and restored by pkg repair.\nBrowser engine porting starts at /SDK/BROWSER_ENGINE_PORT.TXT and targets WPE WebKit with the native browser kept as fallback. Phase 97 adds /bin/browserhost as a deterministic test backend for the host bridge.\n";
...
const SDK_BROWSER_ENGINE_PORT: &[u8] = b"coolOS Browser Engine Port ABI v1\n\nTarget engine: WPE WebKit.\nFallback engine: coolos-native.\n\nGoal\nRun a mature WebKit-class engine for modern sites while keeping the existing native browser as a small fallback/debug surface.\n\nPhase 97 host bootstrap\n- Browser opens /bin/browserhost from browser://engine when the configured target is wpe-webkit.\n- Browser writes /SYSTEM/BROWSER-ENGINE/HOST.REQUEST with load, resize, surface, and input hints.\n- The deterministic browserhost test backend writes /SYSTEM/BROWSER-ENGINE/HOST.READY and /LOGS/BROWSER-ENGINE-HOST.TXT.\n- Normal pages continue to use the native renderer until a real WPE backend is ready and selected.\n\nHost contract\n- Surface output: RGBA buffers presented through the GUI window API for the Phase 97 test backend; future WPE output will map onto the same surface contract.\n- Input: keyboard, pointer, focus, and resize events translated from coolOS GUI events.\n- Network: DNS/TCP/TLS and HTTP paths through the kernel networking stack until a fuller POSIX socket layer exists.\n- Storage: /CONFIG/BROWSER.*, /Downloads, /TMP, /lib shared objects, read-only mmap_file mappings, and package-owned engine files.\n- Fonts: /FONTS with scalable font lookup planned.\n- Process model: browser shell plus isolated engine host process.\n- Threading: ABI 14 exposes shared-address-space userspace threads, futex wait/wake, per-thread FS-base TLS, spawn-with-TLS, pthread_create/join/exit/self, pthread mutex/condvar/once/key APIs, and per-thread errno storage for hosted engine run loops.\n- Dynamic loading: Phase 77 maps bounded /lib ET_DYN dependency graphs, file-backs read-only segments, follows DT_NEEDED sonames, applies RELA relocations, resolves dynsym exports across objects, copies ELF TLS templates, handles TLS relocations, runs init arrays in dependency order, and seals text with W^X mprotect before invocation.\n- File runtime: /TMP, open_flags, fd read/write/close, and read-only mmap_file are available for hosted engine assets and cache probes.\n\nKnown blockers before real WPE boots\n- libc ld.so entry points, symbol versioning, lazy PLT binding, destructors, and broader C/C++ runtime support.\n- Higher address-space budgets plus shared/writable file mappings with dirty-page sync.\n- POSIX socket/open flag compatibility and nonblocking run-loop edge cases.\n- JavaScriptCore JIT policy or interpreter-only engine configuration.\n- Software rendering backend first; EGL/OpenGL/WebGL can come later.\n\nDiagnostics\n- Terminal: engine, engine abi, engine requirements, engine config, engine log, engine recovery.\n- Browser: browser://engine.\n- Sysreport/recovery include browser engine readiness, host state, futex telemetry, TLS thread counts, POSIX pthread shim status, file_mmap readiness, and dynamic-linker readiness.\n";
...
const CF_TOTAL_BLOCKS: u32 = 1536;

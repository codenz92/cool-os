...
USER_BROWSERHOST_TARGET := $(CURDIR)/target/userspace/hello/x86_64-unknown-none/release/browserhost
...
smoke-phase95-hardware-validation: build-usb-image build-usb-safe-image build-usb-secure-image verify-secure-boot-artifacts
	python3 $(CURDIR)/scripts/qemu_smoke.py \
		--artifact-dir "$(SMOKE_ARTIFACT_DIR)" \
		--artifact-name "$@-normal-usb" \
		--uefi \
		--uefi-code "$(QEMU_EFI_CODE)" \
		--boot-disk "$(USB_IMAGE)" \
		--usb-storage \
		--usb \
		--seconds 140 \
		--fw-cmd "hardware;;devices;;sysreport write;;support bundle;;cat /LOGS/HARDWARE.TXT;;cat /LOGS/SUPPORT-BUNDLE.TXT;;flush" \
		--expect "FB 1920x1080" \
		--expect "hardware report_path=/LOGS/HARDWARE.TXT" \
		--expect "hardware readiness framebuffer=ok" \
		--expect "storage root=usb0 layout=gpt-coolfs" \
		--expect "support bundle wrote /LOGS/SUPPORT-BUNDLE.TXT" \
		--expect "support_bundle redacted=yes passwords=excluded private_keys=excluded" \
		--expect "== installer disks ==" \
		--expect "SYSREPORT" \
		--expect "flush: ok" \
		--expect "[boot] desktop ready"
	python3 $(CURDIR)/scripts/qemu_smoke.py \
		--artifact-dir "$(SMOKE_ARTIFACT_DIR)" \
		--artifact-name "$@-safe-usb" \
		--uefi \
		--uefi-code "$(QEMU_EFI_CODE)" \
		--boot-disk "$(USB_SAFE_IMAGE)" \
		--usb-storage \
...
smoke-phase97-browser-engine-host: build
	python3 $(CURDIR)/scripts/qemu_smoke.py \
		--artifact-dir "$(SMOKE_ARTIFACT_DIR)" \
		--artifact-name "$@" \
		--bios "$(BIOS)" \
		...
		--usb \
		--seconds 80 \
		--fw-cmd "browser browser://engine;;engine;;engine log;;sysreport write;;cat /LOGS/SYSREPORT.TXT;;cat /SYSTEM/BROWSER-ENGINE/HOST.READY;;cat /LOGS/BROWSER-ENGINE-HOST.TXT;;flush" \
		--screendump "$(SMOKE_ARTIFACT_DIR)/phase97-browser-engine-host.ppm" \
		--expect-framebuffer-window \
		--expect "browser: opening browser://engine" \
		--expect "browserhost: ready /SYSTEM/BROWSER-ENGINE/HOST.READY" \
		--expect "browserhost: presented test surface" \
		--expect "mode=host-bootstrap" \
		--expect "host_helper=/bin/browserhost present" \
		--expect "host_ready=/SYSTEM/BROWSER-ENGINE/HOST.READY ready" \
		--expect "host_bridge request=/SYSTEM/BROWSER-ENGINE/HOST.REQUEST log=/LOGS/BROWSER-ENGINE-HOST.TXT ipc=file-artifacts surface=rgba-gui input=gui-events" \
		--expect "fallback active=coolos-native reason=wpe-backend-not-ready-test-host-only" \
		--expect "engine_host ready=yes backend=test process=browserhost pid=" \
		--expect "status=ready" \
		--expect "flush: ok" \
		--expect "[boot] desktop ready"
...
smoke-phase71-browser-engine-port: build
	python3 $(CURDIR)/scripts/qemu_smoke.py \
		...
		--expect "browser_engine=host-bootstrap target=wpe-webkit active=coolos-native abi=1" \
		--expect "Target engine: WPE WebKit." \
		--expect "[boot] desktop ready"
